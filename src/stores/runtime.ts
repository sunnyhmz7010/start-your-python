...
export const useRuntimeStore = defineStore("runtime", {
  ...
  getters: {
    canSubmitInput: (state) => state.sessionId !== null,
  ...
  actions: {
    ...
    async detectPython(force = false) {
      if (
        !force &&
        (this.python.available || this.status === "python-missing")
      ) {
        return this.python;
      }
    ...
    async submitInput(input: string) {
      ...
      try {
        ...
        this.terminalOutput += `[Runtime] 发送输入失败: ${formatRuntimeError(
          error
        )}\n`;
      }
    },
    ...
    handleState(payload: PythonStateEvent) {
      ...
      if (payload.message && payload.status !== "running") {
