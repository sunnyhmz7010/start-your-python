...
describe('runtime store', () => {
  ...
  it('starts a run and appends stdout chunks to the terminal buffer', async () => {
    ...
    expect(store.terminalOutput).toContain('Python 3.12.0')
    expect(store.terminalOutput).toContain('C:/Python312/python.exe')
  ...
  it('does not start another process while a python session is running', async () => {
    ...
    expect(store.terminalOutput).toContain('当前已有 Python 程序正在准备或运行')
  })

  it('does not start another process while python detection is in progress', async () => {
    const store = useRuntimeStore()
    let resolveDetection: (value: { available: boolean; command: string }) => void = () => undefined
    const detectionPromise = new Promise<{ available: boolean; command: string }>((resolve) => {
  ...
  it('does not start another process while python is starting', async () => {
    const store = useRuntimeStore()
    let resolveStart: (value: { sessionId: string; command: string }) => void = () => undefined
    const startPromise = new Promise<{ sessionId: string; command: string }>((resolve) => {
  ...
  it('appends final status and exit code when a run completes', async () => {
    ...
    expect(store.lastRunState).toEqual({ sessionId: 'session-1', status: 'completed', exitCode: 0 })
  ...
  it('does not cache final state events for unknown sessions outside startup', () => {
    ...
    runtimeMock.emitState({ sessionId: 'stale-session', status: 'completed', exitCode: 0 })
  ...
  it('appends stopped status after stopping a running session', async () => {
    ...
    await store.stopRun()
    runtimeMock.emitState({ sessionId: 'session-1', status: 'stopped', exitCode: null })
